<template>
  <div>
      ...
      <el-tab-pane label="文章审核" name="article">
        <el-table :data="blogList" style="width: 100%">
          ...
          <el-table-column label="操作" width="180">
            <template slot-scope="scope">
              <el-button type="primary" @click="appeovedBlog(scope.row)"
                >通过</el-button
              >
      ...
      <el-tab-pane label="新用户审核" name="user">
        <el-table :data="userList" style="width: 100%">
          ...
          <el-table-column label="操作">
            <template slot-scope="scope">
              <el-button type="primary" @click="appeovedUser(scope.row)"
                >通过</el-button
              >
              <el-button type="danger" @click="rejectUser(scope.row)"
                >删除</el-button
              >
            </template>
          </el-table-column>
    ...
    <div :style="{display: isshow === 'user'?'block':'none' }">
      <el-pagination
        layout="prev,pager,next"
        :total="blogtotalNum"
        :page-size="serchUserparams.limit"
    ...
    <div :style="{display: isshow === 'article'?'block':'none' }">
      <el-pagination
        layout="prev,pager,next"
        :total="usertotalNum"
        :page-size="serchBlogparams.limit"
        @current-change="handleBlogcrrentchange"
      >
      </el-pagination>
    </div>
  </div>
...
export default {
  ...
  methods: {
    getUserList() {
      identifyService.unapproveduserlist(this.serchUserparams).then((rs) => {
        this.userList = rs.data.data.userlist;
        this.usertotalNum = rs.data.data.totalNum;
      });
    },
    appeovedUser(user) {
      identifyService.approveduser({ key: user.key }).then((rs) => {
    ...
    getBlogList() {
      blogService.unapprovedblog(this.serchBlogparams).then((rs) => {
  
        this.blogList = rs.data.data.rs;
        this.blogtotalNum = rs.data.data.totalNum;
      });
    },
    appeovedBlog(article) {
      // console.log(article);
      blogService.approvedblog({ blogId: article.blogId }).then((rs) => {
        if (rs.data.status === 200) {
          this.$message.success("批准成功");
          this.blogList.splice(article, 1);
        }
      });
    },
    rejectBlog(article) {
